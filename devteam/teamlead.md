...
Two refinements, both 2026-09-23. **The lead publishes and flips go-live without asking** — that moved into the lead's hands with the automated-deployment ruling; only **spending money** and **pointing a domain** remain the owner's, and once he approves either, the lead executes every command itself. And **for websites this boundary is not the authority**: `zeow RS2-M3-websites/CLAUDE.md` **§A** is, and it is more permissive by design — the web-team agents add and edit content and deploy it alone, with delete, noindex, page merge and live URL change going to the owner through the approval bot. §A governs a website; this line governs everything else.
...
- **LEAD-24** Secrets are documented ONCE, before the first build hour — never added piecemeal mid-build. Every project and sub-project (domain or software subdomain) carries its own `.env` at its root, listed in that repo's `.gitignore` (`*.env`). Once the feature set is documented (LEAD-20's approved design), the lead **enumerates every secret the whole feature set will need** — one line per key: name · created (YYYY-MM-DD) · expires (YYYY-MM-DD; when the issuer sets none, a rotation-due date, OPS-24) · deployed-to (every place the value is duplicated — worker secret, Actions secret, Vault key) · lanes (the `M0-laneindex` codes that consume it) — and **asks the product owner to fill every value into `.env` before building starts**. The lead writes the key list first (a committed `.env.example` with the same lines and empty values — the ask list, and the one surface a future expiry/self-healing check reads); the owner copies it to `.env` and fills it; the build gate opens only when every key has a value. A key discovered mid-build is a design miss: it stops the lane, goes up as a question (LEAD-18), and lands in both files in the same turn (LEAD-23) — never a placeholder, never a value pasted into chat or a ticket (MGR-32 · SEC-4). What this prevents: two secrets added on two different days with mismatched expiry or a stale value nobody can trace.
