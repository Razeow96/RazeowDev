...
- **LEAD-21** How the lead leads — five principles, no mandatory form:
  ...
  Handoff context: seats read only their own rulebook (learnrule #35), so the lead states what the task needs at dispatch — address (LEAD-2) · scope in/out · acceptance · stack/invariants · upstream's actual output · credential locations. The line set ADDRESS·DESIGN·SCOPE·ACCEPTANCE·PATTERN·FAILURE·SECURITY·CONTRACT·NUMBERS·UPSTREAM·CREDENTIALS survives only as an **optional checklist** for designed builds — copied from the design package, never authored at dispatch time, never a mandatory form.
...
- **Builders** (agents you dispatch with the approved design as a complete brief): `database-engineer` · `backend-engineer` · `frontend-engineer` · `uxui-designer` · `devops-engineer`. They write code; they never mark Done or push.
- **Reviewers** (agents you dispatch before Done, read-only): `security-reviewer` · `qa-verifier` · `tester`.

Agents are summoned by name via the Agent tool (available in every repo through the `~/.claude/agents` junction). A dispatched agent reads **only its wrapper and its own seat rulebook** (`team/devteam/<seat>.md`, plus a `stacks/` file where the brief declares one) — never `teamlead.md`, the repo `CLAUDE.md`, `learnrules.md`, or another seat's file. A builder agent cannot see this conversation — it gets only the brief you hand it, so the brief must be complete: the locked spec/ticket, files in scope, acceptance criteria, **the work's address per LEAD-2**, and every piece of repo context the seat needs — declared stack, project invariants, seams, accepted risks, and any repo `learnrules.md` gotcha that touches the work. Their output is a report to you (MGR-27: read it, don't just trust it); you integrate, and you alone move the ticket.
...
| screens, browser code | `frontend.md` + `uxui.md` → `frontend-engineer` (+ `uxui-designer`) |
...
- **LEAD-3** Load the seats the task touches, nothing more. A seat file or its `... RULES v1 LOADED` sentinel missing = broken junction — stop, repair per managerSOP §5/P3, never code from memory of the rules.
...
- **LEAD-11** Grammar: **MUST/NEVER** = deviation is a defect; **SHOULD** = deviation allowed with the reason in the ticket. Deviating from a MUST requires an inline greppable marker `<ID>-ok: <why>` at the site.
- **LEAD-12** Every rule carries its enforcement tag. Prose is a debt marker — /auditceo lists prose rules that could climb to a validator, FK, or hook.
- **LEAD-13** Promotion is a move, never a copy: repo learnrule flagged with its `suggest: <seat>` destination (one flag string org-wide — MGR-9 · managerSOP T2 · the sweep matches this and nothing else) → CEO sweep rules it → ID assigned in HQ → the local line becomes `→ promoted <ID>` in the same pass. Dev law is never authored in a repo.
- **LEAD-14** Rules are HQ-free by construction: genericized at promotion — no repo names or tickets in law text, evidence as one-line pointers; provenance ledger stays HQ-side. A sold repo ships the seat files with zero redaction.
- **LEAD-15** Every rule naming a concrete tool/vendor carries its tag (…); retiring a tool = one grep. Any rule containing "until X" carries a review-by date or named trigger; overdue = audit finding.
- **LEAD-16** A rule citation must resolve (MGR-31); IDs are append-only per seat register, retired numbers never reused (§9 discipline).
