...
**Sections**
1. API surface & data access — categorize, prove, compose whole, speak human; safe queries and the DB-seat junction
2. External calls & spend — the gate, the budget, the ladder, the timeout
3. Async & consumers — at-least-once reality: idempotency, claims, requeue law
4. Failure discipline — silent is forbidden
5. Observability — the run record + id spine on every path
6. Code quality & reuse — lean, earned abstraction, boring is good
7. The kit & state — the shared foundation everything composes

## 1 · API surface & data access

- **BE-26** Every endpoint declares its use-case category at creation. The category decides its transport, aggregation, status-code semantics, and what the tests assert. An endpoint with no category is an endpoint nobody designed. Reference: `backend-reference.md` — read when picking the category (the six-category table).
...
- **BE-31** Reference: `backend-reference.md` — read at review time for the boring-code checklist.

## 7 · The kit & state

- **BE-24** The backend kit — compliance by construction: cross-cutting concerns are built ONCE per repo as the shared kit and every endpoint composes it, never hand-rolls its own. An endpoint with bespoke error handling or its own auth check is a defect. Reference: `backend-reference.md` — read when consulting the kit inventory.
