...
- **FE-16** Motion is chosen by what it is, and it never costs the main thread. Reference: `frontend-reference.md` — read when picking the format (the four-way mapping). Anything heavy is lazy-loaded, cached with a long-lived immutable URL (fingerprinted per OPS-19), and never blocks first paint; honour reduced-motion preferences, and pause or drop motion that is off-screen. A decorative animation that delays interaction is a defect.
