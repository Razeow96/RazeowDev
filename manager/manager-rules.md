...
- **MGR-23** Done ≠ live. Deploying built artifacts is part of finishing a build — do it without being asked, **including publishing and the go-live switch** (owner ruling 2026-09-23, reversing "go-live is the owner's"; deployment is automated now and the owner names a gate when he wants one). **Two things still stop and ask: anything that SPENDS money or changes a billing tier, and pointing a domain at a site.** Git pushes remain the owner's. **Approval is for the DECISION, not for the commands** — once the owner says yes, execute it end to end; handing him a command list he could have had run for him is the failure this rule closes. Website AI permissions are governed by `zeow RS2-M3-websites/CLAUDE.md` §A, which this rule never overrides. State a go-live prerequisite once, in its issue, then drop it.
